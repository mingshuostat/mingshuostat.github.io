---
layout: archive
title: ""
permalink: /talks/
author_profile: true
---

## Invited & Conference Talks

- **Graph-based Methods on High-dimensional Missing Data Change-point Detection**  
  - Joint Statistical Meetings (JSM), Toronto, Canada — *Aug 2023*

- **Robust Max Statistics for High-Dimensional Inference**  
  - Southern University of Science and Technology (SUSTech), Shenzhen, China — *Jun 2024*

- **Generalized Independence Test for Modern Data**  
  - Joint Statistical Meetings (JSM), Portland, OR — *Aug 2024*  
  - ICSA Applied Statistics Symposium, Storrs, Connecticut — *Jun 2025*  
  - ICSA China Conference, Zhuhai, China — *Jun 2025*  
  - The 3rd Joint Conference on Statistics and Data Science in China, Hangzhou, China — *Jul 2025*

- **Adaptive Two-Sample Test Using Graph-based Method**  
  - Joint Statistical Meetings (JSM), Nashville, TN — *Aug 2025*

- **A Hypothesis Test for Independent Component Models under the High-Dimensional Regime**  
  - Joint Statistical Meetings (JSM), Boston, MA — *Aug 2026*
